...
public class OreBreakEvent {
    ...
    public void dropSmelting(BlockEvent.HarvestDropsEvent event) {
        ...
        if (Configs.debugMode && event.getHarvester() != null)
            event.getHarvester().sendMessage(new TextComponentString(event.getState().getBlock().getRegistryName().toString()));
        ...
        if (check && event.getHarvester() != null && !event.isSilkTouching() && event.getState().getBlock().getUnlocalizedName().toLowerCase().contains("ore")) {
            dropListReplace(event);
        }
    }

    private void dropListReplace(BlockEvent.HarvestDropsEvent event) {
        ...
        if (!main.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                iterator.remove();
                out.setCount(quantityDropped(fortuneLvl));
